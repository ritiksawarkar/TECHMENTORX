...
export default function Leaderboard({ onClose }) {
  ...
  useEffect(() => {
    load();
    const unsub = useCompilerStore.subscribe(
      (s) => s.leaderboardTick,
      (tick) => {
  ...
  const modal = (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-lg rounded-2xl border border-white/10 bg-gray-900/95 p-6 text-white shadow-2xl">
        ...
        <div
          ref={listRef}
          className="max-h-80 overflow-auto rounded-lg border border-white/10 bg-black/30 p-2"
        >
          {loading ? (
            ...
            <ol className="list-decimal list-inside space-y-2">
              {items.length === 0 && (
                <li className="text-sm text-white/60">No scores yet</li>
              )}
              {items.map((it) => (
