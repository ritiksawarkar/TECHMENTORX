          ...
          <>
            ...
            <div className="ml-auto flex items-center gap-2 opacity-0 group-hover:opacity-100 transition">
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  if (onRenameClick) onRenameClick(itemPath);
                }}
                title="Rename"
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  if (onDeleteClick) onDeleteClick(itemPath);
                }}
                title="Delete"
...
export default function FileExplorer() {
  ...
  const loadStructure = useCallback(async () => {
    ...
    try {
      ...
      console.log("📁 Project Structure (backend filtered):", {
        items: structure.map((i) => i.name),
      });
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("project-structure-changed", onProjectChange);
  }, [loadStructure]);
  ...
  const handleFilePreview = async (filePath, fileName) => {
    try {
      ...
      const opened = (state.tabs || []).find((t) =>
        t.path ? t.path === filePath : t.name === fileName,
      );
      if (opened) {
        // Select existing tab and update content (ensure string)
        if (state.selectTab) state.selectTab(opened.id);
        if (state.updateTabContent)
          state.updateTabContent(opened.id, String(data.content || ""));
      } else {
  ...
  const createFileAndRefresh = async (path, content = "// New file\n") => {
    try {
      ...
      try {
        addRecentlyOpened(path, path.split("/").pop());
      } catch {}
      return true;
    } catch (err) {
  ...
  const handleRename = async (item, newName) => {
    if (newName.trim() && newName !== item.name) {
      try {
        ...
        // If error is 409, inform user that a file/folder with the new name already exists
        if (err.response && err.response.status === 409) {
          toast.push("A file or folder with that name already exists.", {
            type: "error",
          });
        } else {
  ...
  const handleCreateFileAPI = async () => {
    ...
    if (!name) {
      toast.push(
        "Please enter a file name (including extension), e.g. main.cpp",
        { type: "error" },
      );
      return;
    }
    ...
    if (!extMatch) {
      toast.push(
        "File extension required. Please include an extension like .js, .py, .cpp",
        { type: "error" },
      );
      return;
    }
    ...
    if (!allowed.has(ext)) {
      toast.push(
        `Invalid or unsupported extension '.${ext}'. Allowed: ${Array.from(
          allowed,
        ).join(", ")}`,
        { type: "error" },
      );
  ...
  const handleHeaderCreateFile = async () => {
    ...
    const extMatch = name.match(/\.([a-z0-9]+)$/i);
    if (!extMatch) {
      toast.push("Please include a file extension, e.g. main.js", {
        type: "error",
      });
      return;
    }
              ...
              <div className="ml-1">
                <button
                  ...
                  onClick={(e) => {
                    ...
                    try {
                      handleQuickCreateUntitledFile(itemPath);
                    } catch (err) {
                      console.error(err);
                    }
                  }}
                ...
                <>
                  ...
                  <div className="ml-auto flex items-center gap-2 opacity-0 group-hover:opacity-100 transition">
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        setSelectedForRename({ path: itemPath, item });
            ...
            {isExpanded && (
              <div className="pl-4 space-y-0.5 border-l border-white/5">
                ...
                {item.children &&
                  ...
                  renderTree(item.children, itemPath, depth + 1)}
              </div>
            )}
          </div>
        );
  ...
  const handleCreateFileInline = async (folderPath) => {
    ...
    if (!name) {
      toast.push("Please enter a file name including extension", {
        type: "error",
      });
      return;
    }
    ...
    if (
      (node?.children || []).some((i) => i.name === name && i.type === "file")
    ) {
    ...
    try {
      ...
      if (ok) {
        ...
        if (base)
          setExpandedFolders((prev) =>
            Array.from(new Set([...(prev || []), base])),
          );
        // open file in editor
        try {
          await handleFilePreview(rel, name);
        } catch (e) {
          console.error("open after create failed", e);
        }
        setCreatingFileAt(null);
        setCreatingFileDraft("");
      }
      ...
      toast.push(`Failed to create file: ${err.message || err}`, {
        type: "error",
      });
    }
  };
  ...
  const handleQuickCreateUntitledFile = async (folderPath = "") => {
    ...
    try {
      ...
      if (folderPath)
        setExpandedFolders((prev) =>
          Array.from(new Set([...(prev || []), folderPath])),
        );
      ...
      try {
        ...
        setTimeout(() => {
          ...
          try {
            document.querySelector(`input[value=\"${name}\"]`)?.focus?.();
          } catch (e) {}
        }, 40);
      } catch (e) {
  ...
  const handleQuickCreateUntitledFolder = async (parentPath = "") => {
    ...
    let candidate = base;
    let i = 1;
    const existsFolder = (name) =>
      children.some((c) => c.name === name && c.type === "folder");
        ...
        <div className="flex-1 overflow-y-scroll text-sm text-white file-explorer-content">
            ...
            <div className="border-b border-white/10">
              ...
              <div className="space-y-1 px-2 py-1">
                {searchResults.map((result, idx) => (
                  <div
                    key={idx}
                    className="flex flex-col gap-0.5 text-xs text-white/70 hover:text-white/90 p-1.5 rounded hover:bg-white/10 cursor-pointer transition"
                  >
          ...
          {(currentPath.length > 0 || projectStructure.length > 0) && (
            <div>
              <div className="px-4 py-2 text-xs font-semibold uppercase text-white/60 tracking-widest bg-black/20 flex items-center justify-between">
                <div>
                  {currentPath.length > 0 ? "Project Files" : "Root Files"}
                </div>
                {currentPath.length === 0 && (
                    ...
                    {showCreateFolderHeader ? (
                      <div className="flex items-center gap-1">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewFolderNameHeader(e.target.value)
                          }
                          onKeyDown={(e) => {
                      ...
                      <button
                        ...
                        onClick={() => {
                          try {
                            handleQuickCreateUntitledFolder(
                              currentPath.join("/"),
                            );
                          } catch (err) {
                            console.error(err);
