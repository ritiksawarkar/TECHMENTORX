      ...
      if (scope === "files") {
        ...
        setSuggestions((prev) => {
          const same =
            ...
            prev.every((p, i) => p.item.id === newSuggestions[i].item.id);
        ...
        setSuggestions((prev) => {
          const same =
            ...
            prev.every((p, i) => {
              const a =
                p.type === newSuggestions[i].type &&
                (p.item.tab?.id || p.item.item?.id || p.item.id) ===
                  (newSuggestions[i].item.tab?.id ||
                    newSuggestions[i].item.item?.id ||
                    newSuggestions[i].item.id);
              return a;
            });
          return same ? prev : newSuggestions;
        });
